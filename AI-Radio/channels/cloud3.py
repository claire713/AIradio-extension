...
class Cloud(BaseChannel):
	...
	async def play(self):
		...
		if self.connect is None or self.connect.done():
			self.magic_eye.send("cloud",0xFFFF)
			self.stop_ch.clear()
...
#------------------Audio processing functions-------------------------
	...
	async def synthesise(self,text):
		#self.magic_eye.send("cloud",0xF800)
		if self.stop_ch.is_set() or self.ble_disconnect.is_set():
			return
		try:
			async with self.AIclient.audio.speech.with_streaming_response.create(model="gpt-4o-mini-tts", voice=self.voice,input=text,instructions=self.voiceprompt,response_format="pcm",speed=1.25) as response:
	...
	#	self.magic_eye.send("cloud",0xF00)
	...
	async def generate_content(self, prompt):
		"""
		Stream LLM output from OpenAI API and push sentences to TTS queue.
		Captures TTFT, total generation time, and token counts for thesis evaluation.
		"""
		buffer = ""
		...
		try:
			logging.info(f"[{self.name}] | Calling OPENAI LLM")
			async for event in await self.AIclient.responses.create(
				model="gpt-4.1", input=prompt, stream=True
        ):
				if self.ble_disconnect.is_set() or self.stop_ch.is_set():
					print("Generation interrupted by BLE disconnect")
					logging.info(f"[{self.name}] | OPENAI LLM generation stopped")
					break
    ...
    # --- Log metrics ---
		...
		#print(f"[LLM] TTFT={ttft:.3f}s | total={total_time:.2f}s | prompt_tokens={prompt_tokens} | completion_tokens={completion_tokens} | {tokens_per_second:.1f} tok/s")
...
#------------------BLE functions----------------------
	...
	def save_llm_response(self,response):
		with open("llm_response.txt","a",encoding="utf-8",errors="replace") as file:
			file.write(response)
			file.write("\n")
			file.flush()
