 ...
 * File: main.cpp
 * 
 * Raspberry Pi Pico OLED display driver for the Spirit of Radio system.
 * Original codebase developed by Frank Duignan
 * 
 * Extensions by Claire Murphy, May 2026:
 * - display_icon() function for displaying bitmask icons on OLED display
 * - Local, cloud and clear command handling in loop()
 * - icon.h header file containing house and cloud icon bitmasks
 * 
 * 
 * 
 */
...
void display_local(uint16_t x1, uint16_t y1, uint16_t w, uint16_t h, uint16_t colour);
void display_cloud(uint16_t x1, uint16_t y1, uint16_t w, uint16_t h, uint16_t colour);
...
void loop() {
  ...
  // put your main code here, to run repeatedly:
 	 ...
 	 while(angle_index < target_index)
	{
		if (angle_index > 0)
		{
			display_drawLine(xa1[angle_index]+screen_radius,screen_radius-ya1[angle_index],screen_radius+xa2[angle_index],screen_radius-ya2[angle_index],colour);
        	display_drawLine(xa1[angle_index]+screen_radius,screen_radius+ya1[angle_index],screen_radius+xa2[angle_index],screen_radius+ya2[angle_index],colour);
        	display_drawLine(screen_radius-xa2[angle_index],screen_radius-ya2[angle_index],screen_radius-xa1[angle_index],screen_radius-ya1[angle_index],colour);
        	display_drawLine(screen_radius-xa2[angle_index],screen_radius+ya2[angle_index],screen_radius-xa1[angle_index],screen_radius+ya1[angle_index],colour);
		}
    ...
    while(angle_index > target_index)
	{
        ...
        display_drawLine(xa1[angle_index]+screen_radius,screen_radius-ya1[angle_index],screen_radius+xa2[angle_index],screen_radius-ya2[angle_index],0);
        display_drawLine(xa1[angle_index]+screen_radius,screen_radius+ya1[angle_index],screen_radius+xa2[angle_index],screen_radius+ya2[angle_index],0);
        display_drawLine(screen_radius-xa2[angle_index],screen_radius-ya2[angle_index],screen_radius-xa1[angle_index],screen_radius-ya1[angle_index],0);
        display_drawLine(screen_radius-xa2[angle_index],screen_radius+ya2[angle_index],screen_radius-xa1[angle_index],screen_radius+ya1[angle_index],0);
	}
	...
	if (Serial.available())
	{
		...
		else if (ch == EYE_END_DELIMITER)
		{
			...
			if (cmd == "local" )
			{
			display_icon(20, 20, ICON_W, ICON_H,
			 value,
			 display_RGBToWord(0, 0, 0),
			 house_mask);   //call display icon function with house mask
			}
			...
			else if (cmd == "cloud")
			{
			 ...
			 value,
			 display_RGBToWord(0, 0, 0),
			 cloud_mask);   //call display icon function with cloud mask
			}
			...
			//For any other command, from other existing channels
			else 
			{
			colour=SerialIn.substring(0,SerialIn.indexOf(SEPARATOR)).toInt();
			value = SerialIn.substring(SerialIn.indexOf(SEPARATOR)+1).toInt();
			//String type = SerialIn.substring(0, SerialIn.indexOf(SEPARATOR));
			//String typeTwo = SerialIn.substring(SerialIn.indexOf(SEPARATOR) + 1);
		...
		else if (ch == DIAL_END_DELIMITER)
		{
			int32_t  dialindex;
			int32_t dialcolour;		
			dialindex = SerialIn.substring(0,SerialIn.indexOf(SEPARATOR)).toInt();
			dialcolour = SerialIn.substring(SerialIn.indexOf(SEPARATOR)+1).toInt();
...
//display icons for local/cloud channels - developed by adapting the existing display_fillRectangle() function
void display_icon(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
                  uint16_t icon_colour, uint16_t bg_colour,
                  const unsigned char* mask)
{
    display_openAperture(x, y, x+w-1, y+h-1);   
    digitalWrite(A0, 0);                       //indicate next byte command
    SPIDR = 0x2c;                             //send memory write command
    while((SPISR & 2) == 0);                 
    digitalWrite(A0, 1);                     //indicate pixel bytes
    uint32_t pix_c = w * h;                 //total number of pixels in icon
    
	//iterate through each pixel in icon
	for (uint32_t i = 0; i < pix_c; i++)   
    {
        // extract the correct bit from the packed byte array
        uint8_t byte = pgm_read_byte(&mask[i / 8]);     //1 byte = 8 pixels
        uint8_t bit  = (byte >> (7 - (i % 8))) & 1;              
        //asign but colour if it is 1, background colour if it is 0
        uint16_t colour;
        if (bit == 1)
       {
        colour = icon_colour;
    ...
    colour = bg_colour;
		...
		//send high and low bytes of colour to display
		SPIDR = (uint8_t)(colour >> 8);
        while((SPISR & 2) == 0);
